...
#' @param x_na_inf TRUE or FALSE of whether to make NA x_var values infinity with a light grey colour to emphasise them. Defaults to TRUE.
...
#' @export
ggplot_hbar_x_na_inf <- function(data,
                                 x_var,
                                 y_var,
                                 text_var = NULL,
                                 pal = NULL,
  ...
  if(is.null(font_size_body)) font_size_body <- simplevis:::sv_font_size_body(mobile = mobile)

  if (is.null(pal)) pal <- simplevis:::sv_pal(1)
  else pal <- pal[1]
  ...
  x_zero_list <- simplevis:::sv_x_zero_adjust(x_var_vctr, x_balance = x_balance, x_zero = x_zero, x_zero_line = x_zero_line)
  ...
  if (all(x_var_vctr == 0, na.rm = TRUE)) {
    ...
    x_breaks <- simplevis::x_numeric_breaks(x_var_vctr, x_balance = x_balance, x_pretty_n = x_pretty_n, x_trans = x_trans, x_zero = x_zero, mobile = mobile)
...
#' @title Horizontal bar ggplot that is coloured - with x_na_inf support.
...
ggplot_hbar_col_x_na_inf <-
  function(data,
    ...
    if (is.null(pal)) pal <- simplevis:::sv_pal(n_col)
    ...
    if(x_na_inf == FALSE) {
      ...
      if (is.factor(col_var_vctr) & !is.null(levels(col_var_vctr))) {
        labels <- levels(data$col_var2)
        n_col <- length(labels) - 1
      } else {
        labels <- sort(unique(data$col_var2))
        n_col <- length(labels) - 1
      }
...
#' @title Horizontal bar ggplot that is facetted - with x_na_inf support.
...
#' @param x_expand A vector of range expansion constants used to add some padding on the x scale.
#' @param x_labels Adjust the  x scale labels through a function or vector.
#' @param x_na_inf TRUE or FALSE of whether to make NA x_var values infinity with a light grey colour to emphasise them. Defaults to TRUE. Only applicable where facet_scales = "fixed" or "free_y".
...
ggplot_hbar_facet_x_na_inf <-
  function(data,
    ...
    if (facet_scales %in% c("fixed", "free_y")) {
      x_breaks <- simplevis::x_numeric_breaks(x_var_vctr, x_balance = x_balance, x_pretty_n = x_pretty_n, x_trans = x_trans, x_zero = x_zero, mobile = FALSE)
      x_limits <- c(min(x_breaks), max(x_breaks))
